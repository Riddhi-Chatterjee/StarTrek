...
def saveModel():
    with open("models/5mins_"+stock+".pth", "w") as c:
        pass
    checkpoint = {
    "epoch": epoch,
...
with open("settings/5mins_"+stock+".txt", 'r') as s:
    for line in s:
        line = line.split("\n")[0]
        data = line.split(" = ")[1]
        tag = line.split(" = ")[0]
...
if torch.cuda.is_available():
    if ch1.upper() == "Y":
        with open("logs/5mins_"+stock+"_log.out", "w") as lg:
            lg.write("Device name: ")
            lg.write(str(torch.cuda.get_device_name(0))+"\n")
    else:
        with open("logs/5mins_"+stock+"_log.out", "a") as lg:
            lg.write("\nDevice name: ")
            lg.write(str(torch.cuda.get_device_name(0))+"\n")
else:
    if ch1.upper() == "Y":
...
if ch.upper() == "N":
    ...
    if ch1.upper() == "Y":
        with open("models/5mins_"+stock+".pth", "w") as c:
            pass
        with open("logs/5mins_"+stock+"_TrainingLoss.txt", "w") as tl:
            pass
    else:
        FILE = "models/5mins_"+stock+".pth"
    ...
    for i, (inputs, labels, seqLens) in enumerate(train_loader):
        if i == batchNum:
            ...
            if batchNum == printingBatch:
                with open("logs/5mins_"+stock+"_TrainingLoss.txt", "a") as tl:
