import datasetHandler_5mins_Gran
import LSTMdataset
import VS_LSTM
import signal
import sys
...
def saveModel():
    with open("models/5mins_Gran_"+stock+".pth", "w") as c:
        pass
    checkpoint = {
    "epoch": epoch,
    ...
    FILE = "models/5mins_Gran_"+stock+".pth"
...
with open("settings/5mins_Gran_"+stock+".txt", 'r') as s:
    for line in s:
        line = line.split("\n")[0]
        data = line.split(" = ")[1]
        tag = line.split(" = ")[0]
...
dataset = LSTMdataset.LSTMdataset("datasets", "5mins_Gran_"+stock+"_MD.txt")
...
if torch.cuda.is_available():
    if ch1.upper() == "Y":
        with open("logs/5mins_Gran_"+stock+"_log.out", "w") as lg:
            lg.write("Device name: ")
            lg.write(str(torch.cuda.get_device_name(0))+"\n")
    else:
        with open("logs/5mins_Gran_"+stock+"_log.out", "a") as lg:
            lg.write("\nDevice name: ")
            lg.write(str(torch.cuda.get_device_name(0))+"\n")
else:
    if ch1.upper() == "Y":
...
if ch.upper() == "N":
    ds = datasetHandler_5mins_Gran.datasetHandler()
    windowSize = int(input("Enter the windowSize: "))
    ds.createDataset(stock, windowSize)
    ds.shuffleDataset("datasets", "5mins_Gran_"+stock+"_MD.txt")
    ...
    if ch1.upper() == "Y":
        with open("models/5mins_Gran_"+stock+".pth", "w") as c:
            pass
        with open("logs/5mins_Gran_"+stock+"_TrainingLoss.txt", "w") as tl:
            pass
    else:
        FILE = "models/5mins_Gran_"+stock+".pth"
    ...
    for i, (inputs, labels, seqLens) in enumerate(train_loader):
        if i == batchNum:
            ...
            if batchNum == printingBatch:
                with open("logs/5mins_Gran_"+stock+"_TrainingLoss.txt", "a") as tl:
